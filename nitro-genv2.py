...
class Console():        
    def ui(self):
        os.system(f'cls && title Discord Nitro Generator  ^' if os.name == "nt" else "clear")
        print(center(f"""\n\n
    ...
    def run(self):
        ...
        try:
            ...
            if req.status_code == 200:
                ...
                Console().printer(Fore.LIGHTRED_EX, "Yanlis", self.code)
            elif req.status_code == 429:
                # rate = (int(req.json()['retry_after']) / 1000) + 1
                Console().printer(Fore.LIGHTBLUE_EX, "RTlimit", self.code)
                # time.sleep(rate)
            else:
                Console().printer(Fore.LIGHTYELLOW_EX, " Tekrar Deneniyor ", self.code)
                  
        except KeyboardInterrupt:
            Console().ui()
            threading.Lock().acquire()
            print(f"{Fore.LIGHTRED_EX} Stopped > {Fore.RESET}Klavye Yuzunden Gen Durduruldu.")
            os.system('pause >nul')
            exit()
        except:
            # Console().printer(Fore.LIGHTRED_EX, "Yanlis", self.code)
            Console().printer(Fore.LIGHTYELLOW_EX, " Yeniden Deneniyor ", self.code)
        
if __name__ == "__main__":
    Console().ui()
    print(" "+Fore.CYAN + str(Console().proxies_count()) + Fore.RESET + " Şu kadar proxy yüklendi...\n\n")
    DNG = Worker()
